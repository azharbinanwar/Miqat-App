...
import com.example.miqatapp.resources.nafil_prayers
...
import com.example.miqatapp.resources.surahs
...
/**
 * Notifications, built on tiles. A prayer (and Jumu'ah) is one toggle tile; flipping it on grows the same
 * group with its settings. Then Surahs, Nafil prayers, and Dhikr groups. ponytail: mock until scheduler + prefs land.
 */
...
fun NotificationsScreen(onBack: () -> Unit = {}) {
    ...
    // Jumu'ah (expandable, like a prayer)
    var jumuahEnabled by remember { mutableStateOf(false) }
    ...
    Scaffold(
        ...
        Column(
            ...
            AppTileGroup(
                title = stringResource(Res.string.each_prayer),
            ...
            AppTileGroup(
                ...
                modifier = Modifier.fillMaxWidth().animateContentSize(),
                items = buildList {
                    add(AppTileItem(title = stringResource(Res.string.prayer_jumuah), leadingIcon = Lucide.Calendar, trailing = { Switch(checked = jumuahEnabled, onCheckedChange = { jumuahEnabled = it }) }))
                    if (jumuahEnabled) {
                        add(stepperTile(stringResource(Res.string.remind_before), jumuahBefore, min, { jumuahBefore = it }, 15, 120))
                        add(stepperTile(stringResource(Res.string.jamaat_after_start), jumuahJamaat, min, { jumuahJamaat = it }, 5, 60))
                        add(toggleTile(stringResource(Res.string.remind_if_missed), jumuahMissed, { jumuahMissed = it }))
                    }
                },
            )

            AppTileGroup(
                title = stringResource(Res.string.surahs),
                modifier = Modifier.fillMaxWidth().animateContentSize(),
                items = buildList {
            ...
            AppTileGroup(
                title = stringResource(Res.string.nafil_prayers),
                items = listOf(
                    toggleTile(stringResource(Res.string.tahajjud), tahajjud, { tahajjud = it }, stringResource(Res.string.last_third_of_the_night)),
                    toggleTile(stringResource(Res.string.ishraq), ishraq, { ishraq = it }, stringResource(Res.string.mid_morning)),
                ),
            )
